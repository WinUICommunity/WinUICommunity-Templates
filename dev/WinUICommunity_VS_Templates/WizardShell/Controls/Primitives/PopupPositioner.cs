...
namespace iNKORE.UI.WPF.Modern
{
    internal class PopupPositioner : DependencyObject, IDisposable
    {
        ...
        private class PopupSecurityHelper
        {
            ...
            {
                PInvoke.SET_WINDOW_POS_FLAGS flags = PInvoke.SET_WINDOW_POS_FLAGS.SWP_NOZORDER | PInvoke.SET_WINDOW_POS_FLAGS.SWP_NOACTIVATE;
                if (!position)
                {
                    flags |= PInvoke.SET_WINDOW_POS_FLAGS.SWP_NOMOVE;
                }
                if (!size)
                {
                    flags |= PInvoke.SET_WINDOW_POS_FLAGS.SWP_NOSIZE;
                }

                PInvoke.SetWindowPos(Handle, IntPtr.Zero,
                    x, y, width, height, (uint)flags);
            }

            internal unsafe Rect GetWindowRect()
            {
                PInvoke.RECT rect = new PInvoke.RECT(0, 0, 0, 0);
            ...
            private static void GetWindowRect(HandleRef hWnd, [In, Out] ref PInvoke.RECT rect)
            {
            ...
            private static extern bool _GetWindowRect(HandleRef hWnd, [In, Out] ref PInvoke.RECT rect);
